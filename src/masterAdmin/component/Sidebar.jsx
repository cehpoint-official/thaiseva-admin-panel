    ...
    <>
      {links.map((item) => (
        <div
          ...
          {item.badge && (
            <span className='bg-blue-500 p-1 text-slate-50 rounded-lg '>
              {item.badge}
            </span>
          )}
        </div>
      ...
      <>
        <div className='w-[300px] sticky top-0  h-screen '>
          ...
          <div className='p-6 gap-2 flex '>
            <img src={Logo} alt='' />
            <p className='md:text-3xl text-xl text-gray-800 font-bold'>
              Thaiseva
            </p>
          </div>
          {/* header end */}
          ...
          {/* body start*/}
          <div className='flex-grow ps-3'>{LINK_LIST}</div>
          {/* body end*/}
