...
const ChartPage = ({ delivered, onDelivery, canceled }) => {
  ...
  const dataForDoughnut = {
    ...
    datasets: [
      {
        data: [onDelivery, delivered, canceled],
        backgroundColor: [
  ...
  const onDeliveryPercentage = totalOrders
    ? ((onDelivery / totalOrders) * 100).toFixed(0)
    : 0;
  const deliveredPercentage = totalOrders
    ? ((delivered / totalOrders) * 100).toFixed(0)
    : 0;
  const canceledPercentage = totalOrders
    ? ((canceled / totalOrders) * 100).toFixed(0)
    : 0;
    ...
    <>
      <div className="flex items-center justify-between ">
        <div className=" h-[13rem] w-full">
          {onDelivery > 0 || delivered > 0 || canceled > 0 ? (
            <Doughnut data={dataForDoughnut} />
          ) : (
            <p className="text-slate-500 text-lg w-full h-full flex items-center justify-center">
              No data to show
            </p>
          )}
        </div>
        <div className=" flex flex-col items-start justify-start gap-3">
          <div className="flex items-center justify-star ">
            <div className=" text-sm text-slate-700 dark:text-slate-700 w-[6rem]">
            ...
            <div className="flex items-center justify-end gap-2">
              <div className="w-28 bg-gray-200 rounded-full h-2 ">
                <div
                  ...
                  style={{ width: `${onDeliveryPercentage}%` }}
                ></div>
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex  items-center gap-2">
              <div className="w-28 bg-gray-200 rounded-full h-2  ">
                <div
                  className={`bg-[#ff6161] h-2 rounded-full`}
